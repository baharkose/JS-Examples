...
//* JSON NEDİR (JavaScript Object Notation: İşaretleme gösterim)?

//* Arrayler içerisindeki objelere JSON diyoruz. Frontend ve backend kısmında yaygın olarak kullanılır.
//* client ve server arasında giden gelen veriler Json verileridir. En önemli transfer dosyası JSON'dur.

//! neden önemlidir?

//* objelere döngü ile erişim yapabilmemizi sağlar.



//! filter dizinin tümünü döndürür o nedenle map ekledik ki yalnızca title ve authoru alabilelim
//! süslü parantez ile map'in içerisinde obje dönmesini sağladık.

//! returnlu versiyon

const newArray1 = books.filter((b) => b.rating >= 4).map((b) => {
...
//! returnsuz versiyon --> Noah hocanın tercihi :)

const newArray = books.filter((b) => b.rating >= 4).map((b) => ({ title: b.title, author: b.author }));
console.log(newArray);

//! otomatik returning ile yaptık nasıl? süslülerin dışına bşr parantez atarak.
